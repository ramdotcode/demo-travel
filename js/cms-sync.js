...
async function syncCMSData() {
    try {
                        ...
                        ${items.map(item => `
                            <div class="route-card" style="padding: 20px; position: relative; overflow: hidden;">
                                ...
                                <div style="display: flex; gap: 10px;">
                                    <a href="https://wa.me/${perusahaan.whatsapp}?text=Halo Demo Travel, saya mau pesan travel rute ${asal} - ${item.tujuan}" class="btn-booking" style="display: flex; align-items: center; justify-content: center; gap: 8px; flex: 1; padding: 10px; font-size: 0.9rem;">
        ...
        // 6. Sync Packages (New Featured Content)
        const packagesContainer = document.querySelector('.packages-grid');
        if (packagesContainer) {
            const resPack = await fetch('data/packages.json');
            const dataPack = await resPack.json();
            const packages = dataPack.packages || [];

            packagesContainer.innerHTML = packages.map(item => `
                <div class="package-card">
                    <div class="package-img ${item.image ? '' : 'placeholder-img'}" style="${item.image ? `background-image: url('${item.image}');` : ''}"></div>
                    <div class="package-content">
                        <div class="package-tag">${item.tag}</div>
                        <h3>${item.title}</h3>
                        <p>${item.subtitle}</p>
                        <div class="package-footer">
                            <span class="price">${item.price}</span>
                            <a href="${item.link}" class="btn-small">Detail</a>
                        </div>
                    </div>
                </div>
            `).join('');
        ...
        // 7. Sync News (New Featured Content)
        const newsContainer = document.querySelector('.news-grid');
        if (newsContainer) {
            const resNews = await fetch('data/news.json');
            const dataNews = await resNews.json();
            const news = dataNews.news || [];

            newsContainer.innerHTML = news.map(item => `
                <article class="news-card">
                    <div class="news-img ${item.image ? '' : 'placeholder-img'}" style="${item.image ? `background-image: url('${item.image}');` : ''}"></div>
                    <div class="news-content">
                        <span class="news-date">${item.date}</span>
                        <h3>${item.title}</h3>
                        <p>${item.excerpt}</p>
                        <a href="${item.link}" class="read-more">Baca Selengkapnya &rarr;</a>
                    </div>
                </article>
            `).join('');
        }
